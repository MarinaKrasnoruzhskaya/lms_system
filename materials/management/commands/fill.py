...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for user in Command.json_read_data('users_data.json', 'users', 'user'):
            permissions = [Permission.objects.get(id=perm_id) for perm_id in user["fields"]["user_permissions"]] if user["fields"]["user_permissions"] else []
        ...
        course_for_create = []
        for course in Command.json_read_data('materials_data.json', 'materials', 'course'):
        ...
        lesson_for_create = []
        for lesson in Command.json_read_data('materials_data.json', 'materials', 'lesson'):
        ...
        for payments in Command.json_read_data('users_data.json', 'users', 'payments'):
            paid_course = Course.objects.get(pk=payments["fields"]["paid_course"]) if payments["fields"][
                "paid_course"] else payments["fields"]["paid_course"]
            paid_lesson = Lesson.objects.get(pk=payments["fields"]["paid_lesson"]) if payments["fields"][
                "paid_lesson"] else payments["fields"]["paid_lesson"]
